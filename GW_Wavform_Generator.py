...
def main():
    ...
    for i in range(0, N):
        ...
        fig.tight_layout()
        fig.savefig("./Data/Data_Model" + str(i) + ".png",
                    dpi=350,
                    bbox_inches="tight")
...
def ht_respon_TQ(t, tc, phic, mc, eta, dl, thetaS, phiS, iota, psi):
    ...
    Dplus_TQ = (
        ...
        (8 * np.cos(2 * kap) *
         ...
         (3 + np.cos(2 * (phiS - phJ)) *
          (9 + np.cos(2 * thetaS) *
           (3 + np.cos(2 * thJ))) - 6 * np.cos(2 * thJ) *
          (np.sin(phiS - phJ))**2 - 6 * np.cos(2 * thetaS) * (np.sin(thJ))**2 +
          4 * np.cos(phiS - phJ) * np.sin(2 * thJ) * np.sin(2 * thetaS))))

    Dcros_TQ = (np.sqrt(3.0) / 4 *
                (-4 * np.cos(2 * kap) *
                 (np.cos(2 * (phiS - phJ)) * np.cos(thJ) * np.cos(thetaS) +
                  np.cos(phiS - phJ) * np.sin(thetaS) * np.sin(thJ)) +
                 np.sin(2 * kap) *
                 (np.cos(thetaS) *
                  (3 + np.cos(2 * thJ)) * np.sin(2 * (phJ - phiS)) +
    ...
    phip_TQ = -np.arctan(2 * np.cos(iota) * Fcros_TQ /
                         ((1 + (np.cos(iota))**2) * Fplus_TQ))

    # """non-precesnp.sing spinning PN correction to the orbital phase"""
    THETA = eta * (tc - t) / (5 * mc / eta**(3.0 / 5))
    PSI_PN = phic - THETA**(
        5.0 / 8) / eta * (1 + (3715.0 / 8064 + 55.0 / 96 * eta) * THETA**
                          (-1.0 / 4) - 3 * np.pi / 4 * THETA**(-3.0 / 8) +
                          (9275495.0 / 14450688 + 284875.0 / 258048 * eta +
                           1855.0 / 2048 * eta**2) * THETA**(-1.0 / 2))

    # """the frequency-domain amplitude: 1st 60 degree MI with antenna response (contain plus and cross mode) for TianQin"""
    ht_TQ = (-mc * Q_TQ / dl * ((tc - t) / (5 * mc))**(-1.0 / 4) *
...
def ht_respon_LISA(t, tc, phic, mc, eta, dl, thetaS, phiS, iota, psi):
    ...
    lam = 3 * np.pi / 4
    Dplus_LISA = (np.sqrt(3.0) / 64 *
                  (-36 * (np.sin(thetaS))**2 * np.sin(2 * alpha - 2 * lam) +
                   (3 + np.cos(2 * thetaS)) *
                   (np.cos(2 * phiS) *
                    (9 * np.sin(2 * lam) - np.sin(4 * alpha - 2 * lam)) +
                    np.sin(2 * phiS) *
                    (np.cos(4 * alpha - 2 * lam) - 9 * np.cos(2 * lam))) -
                   4 * np.sqrt(3.0) * np.sin(2 * thetaS) *
                   (np.sin(3 * alpha - 2 * lam - phiS) -
                    3 * np.sin(alpha - 2 * lam + phiS))))

    Dcros_LISA = (
        1 / 16 *
        (np.sqrt(3.0) * np.cos(thetaS) *
         (9 * np.cos(2 * lam - 2 * phiS) -
          np.cos(4 * alpha - 2 * lam - 2 * phiS)) - 6 * np.sin(thetaS) *
         (np.cos(3 * alpha - 2 * lam - phiS) +
          3 * np.cos(alpha - 2 * lam + phiS))))

    # """antenna pattern function for '+' mode"""
    Fplus_LISA = (np.cos(2 * psi) * Dplus_LISA -
                  np.sin(2 * psi) * Dcros_LISA) / 2.0

    # """antenna pattern function for '×' mode"""
    Fcros_LISA = (np.sin(2 * psi) * Dplus_LISA +
                  np.cos(2 * psi) * Dcros_LISA) / 2.0
    ...
    # """1st MI polarization phase for TianQin"""
    phip_LISA = -np.arctan(2 * np.cos(iota) * Fcros_LISA /
                           ((1 + (np.cos(iota))**2) * Fplus_LISA))
    ...
    # """the frequency-domain amplitude: 1st 60 degree MI with antenna response (contain plus and cross mode) for TianQin"""
    ht_LISA = (-mc * Q_LISA / dl * ((tc - t) / (5 * mc))**(-1.0 / 4) *
